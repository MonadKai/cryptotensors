...
/// Decode a JWK key material field (`k`, `x`, or `d`) tolerating both of
/// the two forms in common use:
///
/// - **padded standard base64** (`+/=` alphabet) — what upstream
///   cryptotensors has always emitted and still round-trips through
///   `to_jwk()`;
/// - **unpadded base64url** (`-_` alphabet, no `=`) — what RFC 7517
///   prescribes for JWK fields and what JOSE-compliant tooling
///   (including `resultscloud-license-cli`) produces.
///
/// Historically this parser was STANDARD-only, which silently rejected
/// any externally-produced JWK. That failure mode showed up as
/// "Invalid base64 key: Invalid padding" the first time an operator
/// tried to encrypt a model with a key generated by a standards-
/// compliant JOSE library. This helper keeps old artefacts working
/// while accepting modern ones.
pub(crate) fn decode_jwk_b64(s: &str) -> Result<Vec<u8>, base64::DecodeError> {
...
impl KeyMaterial {
    ...
    pub fn new_enc_key(
        ...
        let key_bytes = if let Some(ref b64_str) = key_b64 {
            let bytes = decode_jwk_b64(b64_str).map_err(|e| {
        ...
        match sig_alg {
            SignatureAlgorithm::Ed25519 => {
                let public = if let Some(pub_b64) = public_b64 {
                    let pub_bytes = decode_jwk_b64(&pub_b64).map_err(|e| {
                ...
                let private = if let Some(priv_b64) = private_b64 {
                    let priv_bytes = decode_jwk_b64(&priv_b64).map_err(|e| {
